...
const LUMA_URL = `https://api.lu.ma/calendar/get-items?calendar_api_id=${CALENDAR_API_ID}&period=future&pagination_limit=10`;
...
export async function GET() {
  try {
    const res = await fetch(LUMA_URL, { next: { revalidate: 3600 } });
    ...
    const events: UpcomingEvent[] = entries.map((e) => ({
      id: e.event.api_id,
      name: e.event.name,
    ...
    // An outage must not look like "no events": say so, so the hero can fall
    // back to the weekly rhythm rather than claiming nothing is scheduled.
    console.error("[luma/upcoming] fetch failed", err);
